...
def generate_meal_plan_pdf(meal_plan: MealPlan) -> bytes:
    ...
    if meals:
        ...
        for i, meal in enumerate(meals):
            meal_name = _safe_get(
                meal, ["meal_name", "name", "meal_type"], f"Refeição {i + 1}"
            )
            ...
            if not foods:
                elements.append(
                    Paragraph("Nenhum alimento registrado.", styles["Normal"])
                )
                elements.append(Spacer(1, 2 * mm))
                continue
            ...
            for food in foods:
                ...
                qty = _safe_float(
                    _safe_get(food, ["quantity_g", "quantity", "amount", "grams"], 0)
                )
                kcal = _safe_float(
                    _safe_get(food, ["calories", "kcal", "energy_kcal"], 0)
                )
                prot = _safe_float(_safe_get(food, ["protein_g", "protein", "prot"], 0))
                carb = _safe_float(
                    _safe_get(
                        food,
                        ["carbs_g", "carbohydrates_g", "carbs", "carbohydrates"],
                        0,
                    )
                )
                fat = _safe_float(
                    _safe_get(food, ["fat_g", "fat", "lipids_g", "lipids"], 0)
                )
            ...
            meal_table.setStyle(
                _table_style(has_total_row=True, row_count=len(table_data))
            )
        ...
        elements.append(
            Paragraph("Nenhuma refeição definida neste plano.", styles["Normal"])
        )
