...
def re_embed_all(db_url: str, batch_size: int = 100):
    ...
    with Session(engine) as session:
        foods = (
            session.exec(select(Food)).all()
            if hasattr(session, "exec")
            else session.query(Food).all()
        )
        total = len(foods)
        print(
            f"🔄 Re-embedando {total} alimentos com paraphrase-multilingual-MiniLM-L12-v2...\n"
        )
        ...
        for start in range(0, total, batch_size):
            ...
            for food in batch:
                nutrient = (
                    ...
                    .filter(FoodNutrient.food_id == food.id)
                    .first()
                )
